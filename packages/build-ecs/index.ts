...
const ecsPackage = JSON.parse(loadArtifact(process.env.ECS_PACKAGE_JSON || 'ulla-ecs/package.json'))
...
function emitFile(fileName: string, services: ts.LanguageService) {
  ...
  const ecsPackageECS = loadArtifact(process.env.ECS_PATH || 'ulla-ecs/dist/src/index.js')
  const ecsPackageAMD = loadArtifact(process.env.AMD_PATH || 'ulla-ecs/artifacts/amd.js')
