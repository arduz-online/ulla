...
const packageJson = JSON.parse(
  fs.readFileSync(require.resolve("./package.json")).toString()
);
const isWatching = process.argv.some($ => $ === "--watch");
const instrumentCoverage =
  process.argv.some($ => $ === "--coverage") ||
...
export async function processFile(opt: {
  ...
  const baseFiles =
    (opt.file && [opt.file]) || (opt.files && opt.files[0]) || [];
...
export async function processJson(file: string) {
  ...
  for (let i = 0; i < config.length; i++) {
    ...
    if ($.kind === "RM") {
      ...
      if (!files.length) {
        throw new Error(resolve($.file) + " not found");
      }
...
function ProgressBarPlugin(options: any) {
  ...
  let barFormat =
    options.format ||
    preamble + ":bar" + barRight + chalk.green.bold(" :percent");
  ...
  return new webpack.ProgressPlugin(function(percent, msg) {
    ...
    if (!running) {
      ...
      if (summary) {
        stream.write(
          chalk.green.bold("Build completed in " + buildTime + "\n\n")
        );
