...
async function injectDependencies(
  ...
  console.log(
    `> update ${folder}/package.json (copying dependencies & version)`
  );
  {
...
// async function injectDependencyVersion(
//   folder: string,
...
//         packageJson[target][dep] = version;
//         console.log(`  using dependency: ${dep}@${packageJson[target][dep]}`);
//     });
...
(async function () {
  ...
  await injectDependencies("packages/ulla-ecs", [], false);
